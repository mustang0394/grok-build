...
impl Default for TelemetryConfig {
    fn default() -> Self {
        // FORK: no baked or build-time product-telemetry endpoints in this fork.
        // (Upstream bakes `internal_defaults()` + `GROK_TELEMETRY_BUILD_*` envs here;
        // discarding them keeps endpoint URLs/keys out of the binary entirely.)
...
mod tests {
    ...
    // FORK: product telemetry endpoints are never baked in (see `Default` above).
    #[test]
